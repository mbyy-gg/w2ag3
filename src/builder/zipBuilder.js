 ...
 */
async function forceFixProject(projectRoot) {
    console.log('[FORCEFIX] 🔧 Memulai auto-fix...');
    
    // Fix root build.gradle.kts
    const rootBuildGradleKts = path.join(projectRoot, 'android', 'build.gradle.kts');
    if (fs.existsSync(rootBuildGradleKts)) {
        const rootFixedContent = `allprojects {
    repositories {
        google()
        mavenCentral()
    }
...
tasks.register<Delete>("clean") {
    delete(rootProject.layout.buildDirectory)
        ...
        fs.writeFileSync(rootBuildGradleKts, rootFixedContent);
        console.log('[FORCEFIX] ✅ root build.gradle.kts diganti');
    }
    
    // Fix app/build.gradle.kts
    const appBuildGradleKts = path.join(projectRoot, 'android', 'app', 'build.gradle.kts');
    if (fs.existsSync(appBuildGradleKts)) {
        const appFixedContent = `plugins {
...
android {
    namespace = "com.example.test_webview"
    compileSdk = 36
    ...
    defaultConfig {
        applicationId = "com.buildbyalby.app"
...
flutter {
    source = "../.."
        ...
        fs.writeFileSync(appBuildGradleKts, appFixedContent);
        console.log('[FORCEFIX] ✅ app/build.gradle.kts diganti');
    }
    
    console.log('[FORCEFIX] ✅ Auto-fix selesai!');
}
...
async function buildFromZip(zipPath, projectType, buildType, onProgress) {
    ...
    try {
        ...
        const extractResult = await safeExtractZip(zipPath, tempDir);
                             // Auto fix project setelah ekstrak
await forceFixProject(tempDir);
...
`;
        ...
        if (content.includes('dev.flutter.flutter-plugin-loader')) {
            ...
            // Replace with new ID and ensure 'apply false' if inside plugins block
            // Simple replace might be risky if syntax differs, but usually it's `id "..." version "..."`
            // We replace the ID and assume version is handled or we enforce version too?
            // Safest is to just replace the ID string if we trust the rest.
            // BUT, the old one might NOT have 'apply false'. The new one usually needs it in settings.gradle.

            // Regex to match: id "dev.flutter.flutter-plugin-loader" version "X.Y.Z"
            // Replace with: id "dev.flutter.flutter-gradle-plugin" version "1.0.0" apply false
...
async function fixAppBuildGradle(projectDir, onProgress) {
    ...
    try {
        ...
        // Check minSdkVersion (Upgrade to 21 if lower, required by many modern plugins)
