...
// ========== HANDLER UNTUK INPUT ANGKA (ORDER LIMIT) ==========
bot.onText(/^\d+$/, async (msg) => {
    const chatId = msg.chat.id;
    const state = userStates.get(chatId);
    
    if (state && state.step === 'waiting_limit_amount') {
        const { processLimitInput } = require('./handlers/orderHandler');
        await processLimitInput(bot, chatId, msg, state.userInfo);
